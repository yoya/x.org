 ...
 *	$Id: ptyx.h,v 6.0 1994/06/04 15:44:00 kagotani Rel $
 */
#include "kterm.h"
...
# define SBFCNT		(F_JISX0201_0 + 1) /* number of single byte fonts */
# ifdef KTERM_MBCS
...
#define CursorY(screen,row) ((((row) - screen->topline) * FontHeight(screen)) \
			+ screen->border + (screen->instatus ? 1 : 0))
#else /* !STATUSLINE */
#define CursorY(screen,row) ((((row) - screen->topline) * FontHeight(screen)) \
			+ screen->border)
#endif /* !STATUSLINE */
