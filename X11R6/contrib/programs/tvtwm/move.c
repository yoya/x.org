...
int pulldown;		/* moving window from a pulldown menu */
{
    ...
    if (Scr->OpaqueMove && Scr->OpaqueMoveSaveUnders) {
	XGetWindowAttributes(dpy, window, &wattr);
	old_save_under = wattr.save_under;
	...
	*cancel = False;
    ...
    if (Scr->OpaqueMove && Scr->OpaqueMoveSaveUnders) {
	attr.save_under = old_save_under;
	XChangeWindowAttributes(dpy, window, CWSaveUnder, &attr);
    }
}
...
int panner;
{
    ...
    else if (!Scr->NoRaiseMove && Scr->OpaqueMove)	/* can't restore... */
...
/*
 * RelativeMove() - move a window relative to it's current position.
 *
 */
void
RelativeMove(win, geom)
    TwmWindow	*win;
    char	*geom;
{
    int			res;
    ...
    res = XParseGeometry(geom, &new_x, &new_y, &width, &height);
    if ((res & (XValue | YValue)) != (XValue | YValue)) {
	fprintf(stderr, "%s: ignoring invalid f.relativemove geometry \"%s\"\n",
		ProgramName, geom);
	return;
    }

    XGetGeometry(dpy, win->frame, &JunkRoot, &x, &y, &JunkWidth, &JunkHeight,
		&JunkBW, &JunkDepth);

    new_x += x;
    new_y += y;

    SetupWindow (win, new_x, new_y,
