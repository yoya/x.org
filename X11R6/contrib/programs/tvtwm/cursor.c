...
Pixmap GetCursorGlyph(name, widthp, heightp, shape_mask)
char *name;
int *widthp, *heightp;
Pixmap *shape_mask;
{
    ...
    if ((number=strtol(name, &tmp, 0)), *tmp != '\0') {
	/* it should be a name */
	number = -1;
	for (i = 0; i < sizeof(cursor_names)/sizeof(struct _CursorName); i++) {
	    if (strcmp(name, cursor_names[i].name) == 0) {
		number = cursor_names[i].shape;
		break;
	    }
	}
	if (number < 0) {
	    fprintf(stderr, "%s: cursor glyph %s not known\n", ProgramName,
			name);
	    return None;
	}
    }
    ...
    XQueryTextExtents(dpy, Scr->cursorFont, string, 1, &direction,
			&ascent, &descent, &extent);

    *widthp = extent.rbearing - extent.lbearing;
    *heightp = extent.ascent + extent.descent;
    bm = XCreatePixmap(dpy, Scr->Root, *widthp, *heightp, 1);

    FBF(1, 0, Scr->cursorFont);

    XDrawImageString(dpy, bm, Scr->NormalGC, -extent.lbearing, extent.ascent,
	    string, 1);
	...
	XQueryTextExtents(dpy, Scr->cursorFont, string, 1, 
		      &direction, &ascent, &descent, &extent);

	*widthp = extent.rbearing - extent.lbearing;
	*heightp = extent.ascent + extent.descent;
	mbm = XCreatePixmap(dpy, Scr->Root, *widthp, *heightp, 1);

	FBF(1, 0, Scr->cursorFont);

	XDrawImageString(dpy, mbm, Scr->NormalGC,
		     -extent.lbearing, extent.ascent,
		     string, 1);
