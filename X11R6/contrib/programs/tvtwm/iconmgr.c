...
void CreateIconManagers()
{
    ...
    if (Scr->iconifyPm.pm == None)
    {
	if (Scr->iconifyPm.name) {
#ifdef XPM
	    XpmColorSymbol ctrans[2];

	    ctrans[0].name = "foreground";
	    ctrans[0].value = NULL;
	    ctrans[0].pixel = Scr->IconManagerC.fore;
	    ctrans[1].name = "background";
	    ctrans[1].value = NULL;
	    ctrans[1].pixel = Scr->IconManagerC.back;
#endif

	    Scr->iconifyPm.pm = FindPixmap(Scr->iconifyPm.name,
				&Scr->iconifyPm.width, &Scr->iconifyPm.height,
				&Scr->iconifyPm.isXpm, NULL,
#ifdef XPM
				ctrans, 2,
#else
				NULL, 0,
	...
	if (Scr->iconifyPm.pm == None) {
	    /* Either no name given, or a failed load. */
	    Scr->iconifyPm.pm = XCreatePixmapFromBitmapData(dpy, Scr->Root,
					(char *)siconify_bits, siconify_width,
					siconify_height, Scr->IconManagerC.fore,
					Scr->IconManagerC.back, Scr->d_depth);
	    Scr->iconifyPm.width = siconify_width;
	    Scr->iconifyPm.height = siconify_height;
	    Scr->iconifyPm.mask = None;
	    Scr->iconifyPm.isXpm = FALSE;
	}
    }
    ...
    TwmWindow *tmp_win;
{
    ...
    if (h < (Scr->iconifyPm.height + 4))
	h = Scr->iconifyPm.height + 4;
    ...
    tmp->icon = XCreateWindow (dpy, tmp->w, 5, (int)(h - Scr->iconifyPm.height)/2,
			       (unsigned int) Scr->iconifyPm.width,
			       (unsigned int) Scr->iconifyPm.height,
    ...
    IconMgr *ip;
{
    ...
    if (wheight < (Scr->iconifyPm.height + 4))
	wheight = Scr->iconifyPm.height + 4;
