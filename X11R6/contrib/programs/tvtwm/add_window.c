...
extern Pixmap CreateGrayPixmap();
...
IconMgr *iconp;
{
    ...
    int saveOpaque;			/* save the Scr->OpaqueMove */
    ...
    /* set this up early so that everything knows this window is ours */
	...
	if (tmp_win->flags & TWM_FLAGS_VALID) {
	...
	if (!LookInList(Scr->DontSqueezeIconL, tmp_win->full_name,
			&tmp_win->class))
	    tmp_win->squeeze_icon = (Scr->SqueezeIcon ||
		(short)(int)LookInList(Scr->SqueezeIconL,
				tmp_win->full_name, &tmp_win->class));
	else
	    tmp_win->squeeze_icon = False;
    }
    ...
    tmp_win->icon_title = !Scr->NoIconTitle;
    if (LookInList(Scr->NoIconTitleL, tmp_win->full_name, &tmp_win->class))
        tmp_win->icon_title = False;
    if (LookInList(Scr->IconTitleL, tmp_win->full_name, &tmp_win->class))
    ...
    /*
     * for compatibility reasons, let the virtual colors default
     * to the titlebar colors if no virtual colors have been specified.
     */
    if (Scr->VirtualC.fore != UNKNOWN_PIXEL)
	tmp_win->virtual.fore = Scr->VirtualC.fore;
    else
	tmp_win->virtual.fore = Scr->TitleC.fore;
    if (Scr->VirtualC.back != UNKNOWN_PIXEL)
	tmp_win->virtual.back = Scr->VirtualC.back;
    else
	tmp_win->virtual.back = Scr->TitleC.back;

    if (Scr->VirtualDesktop) {
	GetColorFromList(Scr->TitleForegroundL, tmp_win->full_name, &tmp_win->class,
	    &tmp_win->virtual.fore);
	GetColorFromList(Scr->TitleBackgroundL, tmp_win->full_name, &tmp_win->class,
	    &tmp_win->virtual.back);
	GetColorFromList(Scr->VirtualForegroundL, tmp_win->full_name, &tmp_win->class,
	    &tmp_win->virtual.fore);
	GetColorFromList(Scr->VirtualBackgroundL, tmp_win->full_name, &tmp_win->class,
	    &tmp_win->virtual.back);
    }
    ...
    if (Scr->VirtualDesktop) {
	if (tmp_win->sticky && Scr->StickyAbove)
	...
	tmp_win->virtualWindow = MakeVirtual(tmp_win,
	    ...
	    tmp_win->virtual.back, tmp_win->border);
    }
    else
	tmp_win->root = Scr->Root;
    ...
    if ((!tmp_win->iconmgr) && (w != Scr->Panner))
...
#undef HALF_AVE_CURSOR_SIZE
		    ...
		    if (AddingX + dx >= (Scr->MyDisplayWidth + offsetx))
		      dx = Scr->MyDisplayWidth - AddingX - 1 + offsetx;
		    if (AddingY + dy >= (Scr->MyDisplayHeight + offsety))
		      dy = Scr->MyDisplayHeight - AddingY - 1 + offsety;
		    if (dx > 0 && dy > 0)
		      XWarpPointer (dpy, None, None, 0, 0, 0, 0, dx, dy);
		} else {
		    XWarpPointer (dpy, None, tmp_win->root, 0, 0, 0, 0,
		...
		MoveOutline(tmp_win->root, 0, 0, 0, 0, 0, 0);
	    } 
		...
		if (tmp_win->root == Scr->VirtualDesktop) {
		    maxw += Scr->vdtPositionX;
		    maxh += Scr->vdtPositionY;
		}
     ...
     */
     if ((tmp_win->root == Scr->VirtualDesktop) && 
         (!FromVirtualDesktop && ask_user == FALSE &&
	 (tmp_win->transient || (tmp_win->hints.flags & PPosition))))
    {
    ...
    TwmWindow *tmp_win;
{
     ...
     */
    if (Scr->hilite.bm) {
	pm = XCreatePixmap(dpy, tmp_win->title_w, Scr->hilite.width,
				Scr->hilite.height, Scr->d_depth);
	gcv.foreground = tmp_win->title.fore;
	gcv.background = tmp_win->title.back;
	gcv.graphics_exposures = False;
	gc = XCreateGC(dpy, pm, (GCForeground | GCBackground |
			GCGraphicsExposures), &gcv);
	if (gc) {
	    XCopyPlane(dpy, Scr->hilite.pm, pm, gc, 0, 0,
			Scr->hilite.width, Scr->hilite.height, 0, 0, 1);
	    XFreeGC(dpy, gc);
	} else
	    XFreePixmap(dpy, pm);
    }
    /* No bitmap already read, try reading a {bit,pix}map from disk */
    if ((Scr->hilite.pm == None) && (Scr->hilite.name)) {
	Scr->hilite.pm = FindPixmap(Scr->hilite.name, &Scr->hilite.width,
				&Scr->hilite.height, JunkIsXpm,
				NULL,
#ifdef XPM
				ctrans, 2,
#else
...
#endif
				NULL);
	/* Got a bitmap (now a pixmap).  Reread it as a bitmap and save. */
	if (!(*JunkIsXpm) && Scr->hilite.pm) {
	    Scr->hilite.bm = XmuLocateBitmapFile(ScreenOfDisplay(dpy,
				Scr->screen), Scr->hilite.name, NULL, 0,
				&Scr->hilite.width, &Scr->hilite.height,
				&JunkX, &JunkY);
	}
    }
    /* No pixmap from disk.  Make one. */
    if (Scr->hilite.pm == None) {
	Scr->hilite.pm = XCreatePixmapFromBitmapData (dpy, tmp_win->title_w, 
					       gray_bits, gray_width, 
					       gray_height,
					       tmp_win->title.fore,
					       tmp_win->title.back,
					       Scr->d_depth);
	Scr->hilite.width = gray_width;
	Scr->hilite.height = gray_height;
    }

    if (Scr->hilite.pm) {
	valuemask = CWBackPixmap;
	attributes.background_pixmap = Scr->hilite.pm;
    } else {
	valuemask = CWBackPixel;
	attributes.background_pixel = tmp_win->title.fore;
    }

    valuemask |= (CWBackingStore | CWEventMask);
    attributes.backing_store = NotUseful;
    if (Scr->hiliteLeft.name || Scr->hiliteRight.name)
	attributes.event_mask = (ExposureMask);
    else
	attributes.event_mask = None;
...
/* Free the pixmap so the colors for this window don't get used on the
 * next one... */
    if (Scr->hilite.bm)
	Scr->hilite.pm = None;
    return w;
