 ...
 */
/* 
 * $Header: image.c,v 4.0 88/01/27 16:52:34 toddb Exp $ 
 * $Locker:  $ 
 */
#include "X11/Xlib.h"
#include "resource.h"
    ...
    int byte_order;
{
    ...
    bytes_per_dest = ROUNDUP((long)image->width, 16) >> 3;
    ...
    int byte_order;
{
    ...
    bytes_per_src = ((long)image->width * image->bits_per_pixel) >> 3;
    bytes_per_dest = ROUNDUP((long)image->width * image->bits_per_pixel,
			     2) >> 3;
    length = bytes_per_dest * image->depth;
