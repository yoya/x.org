    ...
    DDXPointPtr pPts;
{
    ...
    if (pSppPts = (SppPointPtr) ALLOCATE_LOCAL((npt + 1) * sizeof(SppPointRec)))
    {
	pSppT = pSppPts;
    ...
    int		fLineEnd, xOrg, yOrg;
{
	 ...
	 * rest of the line */
	if (pPts = (SppPointPtr) ALLOCATE_LOCAL(cpt * sizeof(SppPointRec)))
	{
	    for(i = 1; i < cpt -1; i++)
	    {
