...
/* $Header:setega.c 1.2$ */
/* $ACIS:setega.c 1.2$ */
/* $Source: /ibm/acis/usr/src/X11/server/ddx/ibm/ega/RCS/setega.c,v $ */

#ifndef lint
static char *rcsid = "$Header:setega.c 1.2$";
#endif
...
outb( 0x3c0, 0x30 ) ;
outb( 0x3C0, 0x01 ) ; /* VS.Attr_Mode - P4,P5 for color select */
outb( 0x3c0, 0x31 ) ;
...
outb( 0x3D4, 0x11 ) ;
outb( 0x3D5, /* VS.V_Retrace_End */ 0x2B ) ;
outb( 0x3D4, 0x12 ) ;
...
/* Open Device File */
if ( ( fd = open( "/dev/ega", O_RDWR ) ) < 0 ) {
	perror( "unix_ega_init: can't open /dev/ega, exiting..." ) ;
	exit( 1 ) ;
}
if ( ioctl( fd, EOSETD, &out_emulator ) < 0 )
	perror( "unix_ega_init: error setting output emulator" ) ;

return fd ;
}
#endif

int egaScreenInitHW( index )
register int	index ;
{
#ifndef iAPX286
...
if ( !been_here ) {
	rtScreenFD( index ) = unix_ega_init() ;
	been_here = 1 ;
}
#endif
set_ega_graphics() ;
