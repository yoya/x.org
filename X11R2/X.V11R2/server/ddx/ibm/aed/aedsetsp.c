...
/* $Header:aedsetsp.c 6.0$ */
/* $ACIS:aedsetsp.c 6.0$ */
/* $Source: /vice/X11/src/server/ddx/ibm/aed/RCS/aedsetsp.c,v $ */
    ...
    int			fSorted;
{
	...
	while(ppt < pptLast)
	{
		...
		aedSetScanline(ppt->y, ppt->x, xStart, xEnd, psrc, alu);
	...
	while(ppt < pptLast)
	{
		...
		for(pbox = prgnDst->rects; pbox< pboxLast; pbox++)
		{
			...
			aedSetScanline(ppt->y, pbox->x1, xStart, xEnd, psrc, alu);
		    }
